...
export default function ClientTopNav() {
  ...
  const [notifs, setNotifs] = useState<
    { id: number; concept: string; points: number; created_at: string }[]
  >([]);
  ...
  return (
    <nav className="fixed top-0 left-0 w-full bg-pop-darkGreen/90 backdrop-blur-xl border-b border-white/10 z-50 px-6 lg:px-12 py-4 flex justify-between items-center">
      ...
      <div className="hidden lg:flex items-center gap-10">
        <Link
          href="/puntos"
          className="text-xs font-black text-white hover:text-pop-gold uppercase tracking-widest transition-colors"
        >
          Mis Puntos
        </Link>
        <Link
      ...
      <div className="flex items-center gap-6" ref={menuRef}>
        <div className="relative" ref={notifRef}>
          <button
            ...
            className="text-gray-400 hover:text-pop-gold transition-colors relative"
          >
          ...
          {showNotifs && (
            <div className="absolute top-12 right-0 w-72 bg-pop-cardGreen border border-white/10 rounded-2xl shadow-2xl overflow-hidden py-2 animate-in fade-in slide-in-from-top-2 duration-200">
              ...
              {notifs.length === 0 ? (
                ...
                notifs.map((n) => (
                  ...
                  >
                    ...
                    <span
                      className={`text-xs font-black flex-shrink-0 ml-2 ${n.points >= 0 ? "text-green-400" : "text-pop-orange"}`}
                    >
                      {n.points >= 0 ? "+" : ""}
                      {n.points}
                    </span>
              ...
              <Link
                ...
                className="block px-4 py-2 border-t border-white/5 text-center text-[10px] font-black text-pop-gold uppercase tracking-widest hover:bg-white/5"
              >
                Ver todo
              </Link>
            </div>
          ...
          {showUserMenu && (
            <div className="absolute top-12 right-0 w-56 bg-pop-cardGreen border border-white/10 rounded-2xl shadow-2xl overflow-hidden py-3 animate-in fade-in slide-in-from-top-2 duration-200">
              <div className="px-5 py-3 border-b border-white/5 mb-2">
                ...
                <p className="text-[10px] text-pop-orange font-bold uppercase mt-0.5">
                  POP{" "}
                  {(session?.user?.tier || "Fan").charAt(0).toUpperCase() +
                    (session?.user?.tier || "fan").slice(1)}
                </p>
              </div>
              <Link
                href="/perfil"
                className="flex items-center gap-3 px-5 py-2.5 text-xs font-bold text-gray-400 hover:text-white hover:bg-white/5 transition-colors"
              >
