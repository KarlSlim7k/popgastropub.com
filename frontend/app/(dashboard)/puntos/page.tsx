...
> = {
  ...
  lover: [
    ...
    {
      icon: "celebration",
      title: "Bebida cumpleañera gratis",
      status: "Disponible",
    },
  ...
  vip: [
    ...
    {
      icon: "restaurant",
      title: "Roll gratis cada 5 visitas",
      status: "Activo",
    },
...
export default function PuntosPage() {
  ...
  const [activeTab, setActiveTab] = useState<"beneficios" | "historial">(
    "beneficios",
  );
  ...
  const handleCheckin = async () => {
    ...
    try {
      const res = await fetchWithAuth<any>("/loyalty/checkin", token, {
        method: "POST",
      });
  ...
  useEffect(() => {
    fetchAPI<any[]>("/promociones")
      .then((data) => {
        if (data?.length) setPromo(data[0]);
      })
      .catch(() => {});
  }, []);
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        ...
        setTier(
          tierRes.current_tier
            ? {
                name: tierRes.current_tier.name,
                min_points: tierRes.current_tier.min,
                max_points: tierRes.current_tier.max,
              }
            : null,
        );
        setNextTier(
          tierRes.next_tier
            ? {
                name: tierRes.next_tier.name,
                min_points: tierRes.next_tier.min,
                max_points: tierRes.next_tier.max,
              }
            : null,
        );
        const hist = Array.isArray(historyRes)
          ? historyRes
          : (historyRes.data ?? []);
        setHistory(
          hist.map((h: any) => ({
            id: h.id,
            description: h.concept ?? h.description ?? "",
            points: h.points,
  ...
  const nextTierPoints =
    nextTier?.min_points ??
    (tier?.max_points ? tier.max_points + 1 : userPoints + 1);
  const progress = nextTier
    ? ((userPoints - (tier?.min_points ?? 0)) /
        (nextTier.min_points - (tier?.min_points ?? 0) || 1)) *
      100
    : 100;
      ...
      <header className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6">
        <div>
          ...
          <div className="flex items-center gap-3 mt-4">
            <span className="bg-pop-gold text-pop-black text-[10px] font-black uppercase px-3 py-1 rounded-full tracking-widest">
              {tier?.name?.toUpperCase() || "POP FAN"}
            </span>
        ...
        <div className="text-left lg:text-right space-y-3">
          <p className="text-5xl lg:text-6xl font-black text-pop-gold font-epilogue tracking-tighter leading-none">
          ...
          <button
            ...
            className={`mt-2 px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${checkinDone ? "bg-green-500/10 text-green-400 border border-green-500/20" : "bg-pop-gold text-pop-black hover:bg-pop-lightGold"}`}
          >
            <span className="material-symbols-outlined text-sm align-middle mr-1">
              {checkinDone ? "check_circle" : "location_on"}
            </span>
              ...
              <div className="w-full h-full bg-pop-black rounded-3xl overflow-hidden relative p-8 lg:p-10">
                ...
                <div className="flex flex-col lg:flex-row justify-between gap-10 relative z-10">
                  <div className="flex-1 space-y-8">
                    <div>
                      <span className="text-xs font-black text-pop-gold uppercase tracking-[0.4em]">
                        Digital Membership
                      </span>
                      <h2 className="text-3xl font-black text-white uppercase mt-1">
                        {tier?.name || "POP Fan"} Card
                      </h2>
                    </div>
                    <div className="space-y-1">
                      ...
                      <p className="text-xl font-mono text-white tracking-widest leading-none">
                        #### #### ####{" "}
                        {String(session?.user?.id ?? 0).padStart(4, "0")}
                      </p>
                    </div>
                    <div className="flex gap-10">
                      <div>
                        ...
                        <p className="text-sm font-black text-pop-gold uppercase">
                          {tier?.name || "POP Fan"}
                        </p>
                      </div>
                      <div>
                  ...
                  <div className="bg-white p-4 rounded-2xl flex flex-col items-center gap-2 w-fit mx-auto lg:mx-0 shadow-[0_0_30px_rgba(255,255,255,0.05)]">
                    <QRCodeSVG
                      ...
                      level="M"
                    />
                    <p className="text-[8px] font-black uppercase text-black tracking-[0.2em] leading-none">
                      Check-in at Bar
                    </p>
                  </div>
                </div>
          ...
          <article className="bg-pop-cardGreen p-8 lg:p-10 rounded-3xl border border-white/5 space-y-6">
            <div className="flex justify-between items-end">
              <div>
                ...
                <p className="text-2xl font-black text-white mt-1 uppercase">
                  Hacia {nextTier?.name?.toUpperCase() || "POP ELITE"} 🏆
                </p>
              </div>
              <p className="text-sm font-black text-pop-gold">
                Faltan {Math.max(0, nextTierPoints - userPoints)} pts
              </p>
            </div>
            ...
            <div className="space-y-3">
              <div className="flex justify-between text-[10px] font-black uppercase tracking-widest text-gray-500">
                ...
                <span className="text-pop-gold text-sm">
              ...
              <div className="h-2 bg-pop-black rounded-full overflow-hidden border border-white/5 p-[1.5px]">
                <div
                  className="h-full bg-gradient-to-r from-pop-orange to-pop-gold rounded-full shadow-[0_0_15px_rgba(242,199,119,0.3)] transition-all duration-1000"
            ...
            <nav className="flex gap-8 border-b border-white/5 mb-8">
              {(["beneficios", "historial"] as const).map((tab) => (
                ...
                >
                  {tab}
                  {activeTab === tab && (
                    <div className="absolute bottom-0 left-0 w-full h-0.5 bg-pop-gold" />
                  )}
                </button>
              ))}
            </nav>
              ...
              {activeTab === "beneficios" ? (
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  {benefits.map((b, i) => (
                    <div
                      key={i}
                      className="bg-pop-cardGreen p-6 rounded-2xl border border-white/5 group hover:border-pop-gold/20 transition-all"
                    >
                      <span className="material-symbols-outlined text-pop-gold text-3xl mb-4 group-hover:scale-110 transition-transform">
                        {b.icon}
                      </span>
                  ...
                  {loading ? (
                    ...
                    <div className="bg-pop-cardGreen p-8 rounded-2xl border border-white/5 text-center">
                      ...
                      <p className="text-xs font-bold text-gray-500 uppercase tracking-widest leading-relaxed">
                        Aún no tienes actividad en tu cuenta.
                        <br />
                        ¡Haz tu primera compra para empezar a acumular puntos!
                      </p>
                    </div>
                  ) : (
                    <>
                      {history.slice(0, historyVisible).map((a) => (
                        <div
                          key={a.id}
                          className="bg-pop-cardGreen p-5 rounded-2xl border border-white/5 flex items-center justify-between"
                        >
                          <div className="flex items-center gap-4">
                            <span className="text-[10px] font-black text-gray-500 w-12">
                          ...
                          <span
                            className={`text-xs font-black ${a.type === "earn" ? "text-green-500" : "text-pop-orange"}`}
                          >
                            {a.type === "earn" ? "+" : ""}
                            {a.points}
                          </span>
                      ...
                      {history.length > historyVisible && (
                        <button
                          ...
                          className="w-full py-3 bg-white/5 border border-white/5 rounded-2xl text-[10px] font-black text-gray-400 uppercase tracking-widest hover:text-pop-gold transition-all"
                        >
        ...
        <aside className="lg:col-span-4 space-y-10">
          ...
          <article className="relative h-64 rounded-3xl overflow-hidden group">
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-black via-black/20 to-transparent p-6 flex flex-col justify-end">
              <span className="text-[9px] font-black text-pop-gold uppercase tracking-widest mb-1">
                Promo Activa
              </span>
              <h3 className="text-2xl font-black text-white uppercase leading-none">
                {promo?.nombre || "Descubre nuestras promos"}
              </h3>
              {promo?.descripcion && (
                <p className="text-[10px] text-gray-400 mt-2 font-bold uppercase tracking-widest line-clamp-2">
                  {promo.descripcion}
                </p>
              )}
            </div>
          ...
          <section className="space-y-4">
            ...
            <div className="space-y-2">
              {[
                ...
                {
                  label: "Calificar Mesero",
                  icon: "star",
                  href: "/calificar-mesero",
                },
                {
                  label: "Mis Facturas (CFDI)",
                  icon: "receipt_long",
                  href: "/mis-facturas",
                },
              ].map((link, i) => (
                <a
                  key={i}
                  href={link.href}
                  className="flex items-center justify-between bg-pop-cardGreen p-5 rounded-2xl border border-white/5 hover:bg-white/[0.02] transition-all group"
                >
                  <div className="flex items-center gap-4">
                    <span className="material-symbols-outlined text-pop-gold text-xl">
                      {link.icon}
                    </span>
              ...
              >
                <div className="flex items-center gap-4">
                  <span className="material-symbols-outlined text-red-500/50 group-hover:text-red-500 text-xl transition-colors">
                    logout
                  </span>
                  <span className="text-xs font-black text-white uppercase tracking-tight">
          ...
          <article className="bg-pop-cardGreen p-8 rounded-3xl border-2 border-white/5 border-dashed text-center">
            ...
            <p className="text-xs font-bold text-gray-500 uppercase tracking-widest leading-relaxed">
              ¿Necesitas ayuda con tus puntos?
            </p>
            <a
              href="https://wa.me/522821278014?text=Hola%2C%20necesito%20ayuda%20con%20mis%20POP%20Points"
              target="_blank"
              rel="noopener noreferrer"
              className="text-pop-gold text-[10px] font-black uppercase mt-4 underline underline-offset-4 decoration-pop-gold/30 inline-block"
            >
