    ...
    fetchWithAuth<DashboardData>("/api/admin/dashboard", session.token)
      .then((data) => {
        setKpiData([
          ...
          {
            label: "Meseros",
            value: data.totalMeseros?.toLocaleString() || "0",
            trend: "Staff",
            trendUp: true,
            icon: "restaurant",
            color: "pop-orange",
          },
          {
            ...
            value: data.totalFacturas?.toLocaleString() || "0",
            trend: "Histórico",
            trendUp: true,
            icon: "receipt_long",
          ...
          {
            label: "Facturas Hoy",
            value: data.facturasHoy?.toLocaleString() || "0",
            trend: "Hoy",
            trendUp: true,
            icon: "today",
            color: "pop-gold",
          },
          {
            ...
            value: data.totalPuntos?.toLocaleString() || "0",
            trend: "Acumulado",
            trendUp: true,
            icon: "token",
            color: "pop-light-gold",
          },
          {
            ...
            value: data.usuariosNuevosMes?.toLocaleString() || "0",
            trend: "Este mes",
            trendUp: true,
            icon: "person_add",
            color: "pop-orange",
            ...
            {barData.map((d, i) => (
              <div key={i} className="flex flex-col items-center flex-1 min-w-[30px] group">
                <div className="w-full relative h-48 bg-white/5 rounded-t-lg overflow-hidden">
                  <div
                    className="absolute bottom-0 left-0 right-0 bg-pop-gold group-hover:bg-pop-lightGold transition-all rounded-t-lg shadow-[0_0_15px_rgba(242,199,119,0.2)]"
                ...
                <tr key={idx} className="hover:bg-white/[0.01] transition-colors">
                  <td className="py-5 px-6">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-pop-gold text-pop-black flex items-center justify-center font-black text-[10px]">{w.initials}</div>
          ...
          {topWaiters.map((w, idx) => (
            <article key={idx} className="bg-[#1C1B1B] p-4 rounded-xl border border-white/5 flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div className="w-10 h-10 rounded-full bg-pop-gold text-pop-black flex items-center justify-center font-black text-xs">{w.initials}</div>
                <div>
