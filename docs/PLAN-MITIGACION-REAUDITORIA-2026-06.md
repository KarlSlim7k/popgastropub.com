...
- [x] REM-02: CSP enforce en vivo (dual-header, política ampliada para Elfsight/Google Fonts) — PR #9, mergeado y desplegado, `curl -sI` confirma ambos headers (`Content-Security-Policy` enforce + `Content-Security-Policy-Report-Only` estricta como telemetría). Walkthrough automatizado (Playwright, incógnito) sobre las 14 rutas públicas: 0 violaciones enforce tras PR #10, que corrigió 2 hallazgos (mapa de `/ubicacion` roto por `frame-src`/redirect a `www.google.com`, y beacon `static.cloudflareinsights.com` bloqueado en script-src/connect-src en todas las páginas). Walkthrough autenticado (Playwright, cuentas demo) sobre las 8 rutas `(dashboard)` (cliente), 9 rutas `(staff)` (mesero) y 14 rutas `(admin)` (admin): login OK en los 3 roles, las 31 páginas protegidas devuelven 200 con 0 violaciones CSP (enforce y report-only). Cero funcionalidad rota. Checklist completo.
